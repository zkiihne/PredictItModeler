...
def kde_fun(points, sofar, start, bw=0.6):
    ...
    plt.plot(x, norm_pdf)
    plt.show()
    return x, kde_pdf, [sum(kde.evaluate(b1)),sum(kde.evaluate(b2)),sum(kde.evaluate(b3)),sum(kde.evaluate(b4)),sum(kde.evaluate(b5)),sum(kde.evaluate(b6)),sum(kde.evaluate(b7)),sum(kde.evaluate(b8)),sum(kde.evaluate(b9))]
...
def generate_current_prices(weeks, currtime,start, plots=False):
    ...
    kx, kde, kde_arr = kde_fun(weekly_averages, initial, start, 0.7)
...
def generate_single_price(name_of_account,current_count, given_time, we, market, starting):
    ...
    norm, kde = generate_current_prices(wks, currtime,starting, plots=True)
    kde_yes = []
    kde_no = []
    for n in kde:
        kde_yes.append(n)
...
if __name__ == '__main__':
    # generate_single_price("VP", 12, 302 , 512, 5993, 44)

    generate_single_price("POTUS", 94, 507, 512, 5994, 84)
