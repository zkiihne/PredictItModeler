...
# makes a dictionary of all of the weeks
...
# helper function to convert a time to a number ie friday noon becomes 512
...
# helper to convert the other way ie 513 becomes "Fri"
def num_to_day(num):
...
# cuts of end of weeks so that only complete ones are included
...
# returns the hour based off the time given
...
# sepreates the list of tweet times into week portions
...
# gets the market number from predict it
def get_market_number(market):
    market_title = ""
    if market == "VP":
        market_title = "How many tweets will @vp post"
    if market == "aoc":
        market_title = "How many tweets will @AOC post"
    if market == "potus":
        market_title = "How many tweets will @potus post"
    if market == "whitehouse":
        market_title = "How many tweets will @whitehouse post"
    file = urlopen("https://www.predictit.org/api/marketdata/all/")
    js = file.read()
    text = js.decode('utf-8')  #
    response = json.loads(text)
    market_id = 0
    market_start = 0
    for md in response["markets"]:
        if str(md["name"]).startswith(market_title):
            market_id = md["id"]
            for contract in md["contracts"]:
                if str(contract["name"]).__contains__("fewer"):
                    if str(contract["name"])[2] == ' ':
                        market_start = int(str(contract["name"])[0] + str(contract["name"])[1])
                    else:
                        market_start = int(str(contract["name"])[0] + str(contract["name"])[1] + str(contract["name"])[2])
    return market_id, market_start

# returns the prices of a given contest number
...
def get_count(qname, end_of_week=512):
    global name
    name = qname
...
# returns a normal distribution based off a set points
def norm(points):
    return scipy.stats.norm(statistics.mean(points), statistics.stdev(points))

# returns the time of the tweets for a given user
def run(id):
    parsed_times = []
